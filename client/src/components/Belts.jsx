...
const ROW_HEIGHT =
  (CHART_HEIGHT - PADDING * 2 - GAP * (ROW_COUNT - 1)) / ROW_COUNT;
...
const Belts = ({ belts = [], onEditBelt, onDeleteBelt }) => {
  ...
  belts.forEach((record) => {
    ...
    const isNewer =
      !existing ||
      new Date(record.date_of_issue) > new Date(existing._rawDate || 0);
    if (isNewer) {
  ...
  const layoutColumn = (rows) => {
    ...
    return rows.map((row) => {
      ...
      if (top < prevBottom + CALLOUT_MIN_SPACING)
        top = prevBottom + CALLOUT_MIN_SPACING;
      prevBottom = top + CALLOUT_HEIGHT;
      return { ...row, calloutTop: top };
    });
  };

  const column0Callouts = layoutColumn(column0Active).map((c) => ({
    ...c,
    column: 0,
  }));
  const column1Callouts = layoutColumn(column1Active).map((c) => ({
    ...c,
    column: 1,
  }));
  ...
  const contentHeight = Math.max(
    CHART_HEIGHT,
    column0Bottom + PADDING,
    column1Bottom + PADDING,
  );
  ...
  return (
    <div ref={wrapperRef} className="mt-3 w-[80%]">
      <div
        style={{
          position: "relative",
          width: "100%",
            ...
            return (
              <div
                ...
                style={{
                  ...
                  border:
                    belt.name === "white"
                      ? `1px solid ${isDark ? "rgba(255,255,255,0.15)" : "#d9d9d6"}`
                      : "none",
        ...
        {callouts.map(({ belt, details, calloutTop, column }, i) => (
          ...
          >
            ...
            >
              <Award
                size={10}
                color={belt.hex === "#fffffc" ? "#808080" : belt.hex}
                strokeWidth={2.5}
              />
            </div>
            <div
            ...
            >
              ...
              >
                {details.certification_no && (
                  <span className="font-bold uppercase tracking-tight text-gray-700 dark:text-gray-500">
                    Certificate No :{details.certification_no || "—"}
                  </span>
                )}
              </span>
