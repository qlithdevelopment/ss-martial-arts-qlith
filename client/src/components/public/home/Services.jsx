...
import {
  servicesCategories as categories,
  allServices,
} from '../../../data/homeData'
...
const Services = () => {
  ...
  const displayServices = isMobile
  ...
  return (
    <section
      id="services"
      className="relative w-full min-h-screen overflow-hidden flex flex-col md:flex-row bg-[#f8f9fa]"
    >
      ...
      <div className="w-full md:w-[30%] lg:w-[25%] bg-primary h-auto md:min-h-screen flex flex-col justify-center px-6 md:px-8 lg:px-10 py-12 md:py-0 relative shrink-0 z-20 shadow-2xl">
        <div className="relative z-10 w-full">
          <div className="flex items-center gap-4 mb-4">
            ...
            <h3 className="text-[var(--color-primary2)] font-bold tracking-[0.2em] uppercase text-sm">
              Choose Your Path
            </h3>
          </div>
          ...
          <div className="flex flex-row md:flex-col gap-3 md:gap-4 overflow-x-auto md:overflow-visible pb-4 md:pb-0 hide-scrollbar w-full">
            {categories.map((cat) => (
              <button
                ...
                className={`text-left text-xs md:text-sm font-bold uppercase tracking-widest px-4 md:px-5 py-3 md:py-4 transition-all duration-300 whitespace-nowrap md:whitespace-normal border-l-4 rounded-r-lg w-full ${
                  activeCategory === cat
            ...
            {displayServices.map((service, index) => (
              ...
              >
                ...
                <div className="flex flex-col p-4 md:p-5 bg-white flex-1">
                  {/* Title */}
                  <h3 className="text-sm md:text-base font-black text-[var(--color-primary)] uppercase tracking-tight mb-1 leading-tight line-clamp-2">
                    {service.title}
                  </h3>
                  ...
                  <Link
                    ...
                    className="w-full mt-auto bg-[var(--color-primary)] text-white py-2.5 text-[10px] font-bold uppercase tracking-[0.15em] hover:bg-[#0b1b24] hover:-translate-y-1 hover:shadow-[0_10px_20px_rgba(38,192,255,0.3)] transition-all duration-300 rounded-lg text-center flex items-center justify-center"
                  >
                    Explore Services
                  </Link>
                </div>
        ...
        <div className="w-full flex justify-center md:block md:w-auto md:absolute md:bottom-10 md:right-10 z-30 pt-4 md:pt-0">
          <Link to="/services">
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-[var(--color-primary)]/10 to-transparent opacity-0 group-hover:opacity-100 rounded-full transition-opacity duration-300" />

              <span className="text-[#0b1b24] font-black text-[10px] md:text-xs tracking-[0.2em] uppercase relative z-10">
                View More
              </span>

              <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-[var(--color-primary)] flex items-center justify-center relative z-10 group-hover:bg-[#0b1b24] transition-colors duration-300">
                <svg
