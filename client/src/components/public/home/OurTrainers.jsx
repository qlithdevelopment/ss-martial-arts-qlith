...
// ─── Skeleton Card ────────────────────────────────────────────────────────
function TrainerSkeletonCard() {
  return (
    <div className="w-screen h-full flex flex-col md:flex-row items-center relative flex-shrink-0 animate-pulse">
      {/* SPLIT BACKGROUND */}
      <div className="absolute inset-y-0 right-0 w-1/2 bg-white/5 z-0" />
      ...
      <div className="w-full md:w-1/3 flex flex-col justify-center px-6 md:pl-32 md:pr-4 h-[30vh] md:h-full relative z-20 pt-12 md:pt-0">
        ...
        <div className="h-10 md:h-16 w-48 md:w-64 bg-white/10 rounded mb-4" />
        <div className="h-[2px] bg-white/10 mb-4 w-[40px]" />
      ...
      {/* CENTER IMAGE */}
      <div className="w-full md:w-1/3 h-[40vh] md:h-[90vh] flex items-end justify-center relative z-30 mt-auto">
        <div className="w-40 md:w-64 h-full max-h-[80%] bg-white/10 rounded-t-xl" />
      </div>
      ...
      <div className="w-full md:w-1/3 flex flex-col justify-center px-6 md:pl-16 md:pr-8 h-[30vh] md:h-full relative z-20 pb-12 md:pb-0">
        ...
        <div className="h-5 w-40 bg-black/10 rounded mb-6" />
        <div className="h-[2px] w-12 bg-black/10 mb-6" />
...
const OurTrainers = () => {
  ...
  useEffect(() => {
    const fetchTrainers = async () => {
      try {
        const res = await api.get('/trainers?per_page=10');
      ...
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        ...
        <motion.div
          style={{
            x,
            width: loading
              ? `${SKELETON_COUNT * 100}vw`
              : `calc(${trainers.length * 100}vw + 40vw)`
          }}
        ...
        >
          {loading ? (
            Array.from({ length: SKELETON_COUNT }).map((_, i) => (
              ...
              <TrainerSkeletonCard key={i} />
            ))
          ) : (
            <>
              {trainers.map((current) => (
