...
import { Plus, Edit2, Trash2, User as UserIcon, Search } from 'lucide-react';
...
const Trainers = () => {
  ...
  const fetchTrainers = async () => {
    try {
      ...
      console.error(error);
      toast.error('Failed to load trainers (Backend might not be ready yet)');
    } finally {
      setLoading(false);
    }
  };
          ...
          {filteredTrainers.map((trainer) => (
            <div key={trainer.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col group">
              ...
              <div className="relative h-48 bg-gray-100 overflow-hidden">
                {trainer.image ? (
                  <img src={`http://127.0.0.1:8000/storage/${trainer.image}`} alt={trainer.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                ) : (
              ...
              <div className="p-5 flex-1 flex flex-col">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
                  <button onClick={() => openEditModal(trainer)} className="flex items-center gap-1.5 text-sm font-bold text-blue-600 hover:text-blue-700 transition-colors bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg">
                    <Edit2 size={14} /> Edit
                  </button>
                  <button onClick={() => handleDelete(trainer.id)} className="flex items-center gap-1.5 text-sm font-bold text-red-600 hover:text-red-700 transition-colors bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg">
